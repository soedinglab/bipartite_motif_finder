...
def create_parser():
    ...
    group.add_argument('--BGsequences', type=str, default=None, help='background sequences (only relevant for training, must have the same format as input sequences).')
    group.add_argument('--predict', action="store_true", default=False, help='use this flag to run the prediction mode.')
    parser.add_argument('--input_type', action="store", default='fasta', choices=['fasta','fastq','seq'], help='format of input files. Can be "fasta", "fastq", and "seq". [Default:"fasta"]')
    parser.add_argument('--model_parameters', action="store", type=str, default=None, help='path to .param file that specifies model parameters. (only relevant for predicting)')
    parser.add_argument('--motif_length', action="store", type=int, default=3, help='the length of each core in the bipartite motif [Default:3]')
    parser.add_argument('--no_tries', action="store", type=int, default=5, help='the number of times the program is run with random initializations (only relevant for training) [Default:5]')
    parser.add_argument('--output_prefix', action="store", type=str, default=None, help='output file prefix. You can specify a directory e.g. "--output_prefix output_dir/my_prefix" [Default:"bipartite"]')
    parser.add_argument('--var_thr', action="store", type=float, default=0.03, help='variability threshold condition to stop ADAM (only relevant for training) [Default:0.03]')
    parser.add_argument('--batch_size', action="store", type=int, default=512, help='the number of sequences processed in each batch of stochastic gradient descent. (only relevant for training) [Default:512]')
    parser.add_argument('--max_iterations', action="store", type=int, default=1000, help='max number of iterations before stopping ADAM. We recommend values between 500 and 1500 (only relevant for training) [Default:1000]')
    parser.add_argument('--no_cores', action="store", type=int, default=4, help='the numbers of CPU cores used. [Default:4]')
    return parser
